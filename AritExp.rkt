...
(define-peg divisoes (and (name s1 (or parenteses number variable)) (? (and "/" (name s2 divisoes)))) (if s2 (cons s1 s2) (cons s1 '())))
(define-peg produtos (and (name s1 divisoes) (? (and "*" (name s2 produtos)))) (if s2 (prod (normDiv s1) s2) (normDiv s1)))
(define-peg subtracoes (and (name s1 produtos) (? (and "-" (name s2 subtracoes)))) (if s2 (cons s1 s2) (cons s1 '())))
(define-peg somas (and (name s1 subtracoes) (? (and "+" (name s2 somas)))) (if s2 (soma (normSub s1) s2) (normSub s1)))
